...
- `character_bible`: the visual consistency document. Schema:
    {style, palette, world, characters, recurring_motifs, voice_fingerprint}.
    Each entry in `characters` is a profile with fields
    {appearance, role, voice_traits, age_or_era}. For your prompt, ONLY the
    `appearance` field matters — role/voice/age are narrative context that
    the illustration model cannot use. `voice_fingerprint` is irrelevant
    to image generation; ignore it.
...
3. Include the relevant `character_bible.characters[name].appearance` text verbatim
   for any character who appears in this scene. Do not include the role,
   voice_traits, or age_or_era fields — they are not visual.
